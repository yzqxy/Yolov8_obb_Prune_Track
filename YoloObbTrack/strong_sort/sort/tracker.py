...
class Tracker:
    ...
    def update(self, detections, classes, confidences):
        ...
        for track_idx in unmatched_tracks:
            # print('unmatched_tracks', self.tracks[track_idx].state)
            self.tracks[track_idx].mark_missed()
        for detection_idx in unmatched_detections:
            self._initiate_track(detections[detection_idx], classes[detection_idx], confidences[detection_idx])
    ...
    def _match(self, detections):
        ...
        #如果框是is_confirmed状态，既track里的state=2，则放到confirmed_tracks队列中，否则放入unconfirmed_tracks中进行匹配，一直存储在unconfirmed_tracks，直到30帧没匹配上再删除
        confirmed_tracks = [
    ...
    def _initiate_track(self, detection, class_id, conf):
        self.tracks.append(Track(
            detection.to_xyah(), detection.rbox, self._next_id, class_id, conf, self.n_init, self.max_age, self.ema_alpha,
            detection.feature))
        self._next_id += 1
